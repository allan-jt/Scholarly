...
async def fetch(query_fields: dict, query_config: dict) -> dict:
    ...
    id_list = query_fields.pop(
        "id", None
    )  # id_list is handled separately from other query fields
    boolean_operator = query_config.pop(
        "boolean_operator"
    )  # boolean_operator is only used for query construction
    ...
    query_params = {"search_query": search_query, "id_list": id_list, **query_config}
    query_string = "&".join(
